...
function normalizeComponentModule(module: any) {
    if (module?.default?.createComponent) return module.default;
    if (module?.createComponent) return module;
    return null;
}
...
export const LowcodePlatform = {
    ...
    async registerComponent(name: string, module: any) {
        const mod = normalizeComponentModule(module);
        if (!mod) {
        ...
        if (mod.registerRuntimeDeps && typeof mod.registerRuntimeDeps === 'function') {
            console.log(`[LowcodePlatform] Registering runtime dependencies for ${name}`);
            mod.registerRuntimeDeps(platformApi);
        }

        if (mod.loadRuntimeDeps && typeof mod.loadRuntimeDeps === 'function') {
            console.log(`[LowcodePlatform] Loading runtime dependencies for ${name}`);
            try {
                await mod.loadRuntimeDeps(() => platformApi);
            } catch (err) {
        ...
        ComponentRegistry.register(name, mod.createComponent, { name });
